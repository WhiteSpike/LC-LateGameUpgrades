...
namespace MoreShipUpgrades.Managers
{
    public class UpgradeBus : NetworkBehaviour
    {
        ...
        internal void AlterStoreItems()
        {
            ...
            if (!PluginConfiguration.BARBED_WIRE_ENABLED.Value)
            {
                logger.LogInfo("Removing Barbed Wire from store.");
                Items.RemoveShopItem(ItemsToSync["Barbed Wire"]);
            }
            else if (ItemsToSync["Barbed Wire"].creditsWorth != PluginConfiguration.BARBED_WIRE_PRICE.Value) Items.UpdateShopItemPrice(ItemsToSync["Barbed Wire"], PluginConfiguration.BARBED_WIRE_PRICE.Value);
        }
